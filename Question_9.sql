...
NEXT STEPS:
Add DISTINCT lgid
WHERE am.lgid = 'AL' OR am.lgid = 'NL' AND am.awardid = 'TSN%'
*/
--Basic Data Query:
SELECT DISTINCT(p.playerid), p.nameFirst, p.nameLast, am.lgid, am.awardid, t.name
	FROM people AS p
	...
	JOIN teams AS t
	USING (teamid)
	WHERE am.awardid = 'TSN%'
	AND am.lgid = 'AL' OR am.lgid = 'NL'
ORDER BY p.playerid, awardid DESC;

--TEST QUERY:
SELECT DISTINCT(p.playerid), p.nameFirst, p.nameLast, am.lgid, am.awardid, t.name,
	CASE WHEN awardid = 'TSN Manager of the Year'
	AND am.lgid = 'AL'
	OR am.lgid = 'NL'
	THEN 'www'
	END
